    ...
    struct RootView: BaseView {
        ...
        var overridesView: some View {
            Form {
                ...
                // 🟢 NEU: Präzisions-Slider mit Haptik & Slot-Machine Zahlen
                Section {
                    VStack(spacing: 15) {
                        Text("\(formatter.string(from: state.percentage as NSNumber) ?? "") %")
                            ...
                            // Slot-Machine Effekt für weiches Scrollen der Zahl
                ...
                header: { Text("Insulin Anpassung") }
                footer: {
                    Text("Nutze die + und - Buttons für präzise 1%-Schritte oder ziehe den Slider für grobe Anpassungen.")
                }
                ...
                Section {
                    ...
                    if state.advancedSettings {
                        ...
                        HStack {
                            Text("SMB Minutes")
                            DecimalTextField("0", value: $state.smbMinutes, formatter: formatter, liveEditing: true)
                            Text("minutes").foregroundColor(.secondary)
                        }
                        HStack {
                            Text("UAM SMB Minutes")
                            DecimalTextField("0", value: $state.uamMinutes, formatter: formatter, liveEditing: true)
                            Text("minutes").foregroundColor(.secondary)
                        }
                        ...
                        if state.overrideMaxIOB {
                            HStack {
                                Text("Max IOB")
                                DecimalTextField("0", value: $state.maxIOB, formatter: insulinFormatter, liveEditing: true)
                ...
                // 🟢 NEU: Prominente, große Action-Buttons
                Section {
                    VStack(spacing: 12) {
                        if !isEditingPreset {
                            Button {
                                ...
                                alertSring = (formatter.string(from: state.percentage as NSNumber) ?? "100") + "%, " +
                                    (
                                        state.duration > 0 && !state
                                            ._indefinite ? (dateFormatter.string(from: duration) ?? "") :
                        ...
                        Button {
                            ...
                            Text(isEditingPreset ? LocalizedStringKey("Save") : LocalizedStringKey("Save as Profile"))
                                ...
                                .padding()
                                .background(
                                    isEditingPreset ? Color
                                        .orange : (unChanged() ? Color.gray.opacity(0.3) : Color.orange)
                                )
                                .foregroundColor(.white)
                                .cornerRadius(12)
                        }
